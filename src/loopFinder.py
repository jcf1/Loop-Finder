...
def is_valid(start_str, end_str, minLen, maxLen):
    start_obj = datetime.strptime(start_str, '%H-%M-%S.%f')
    end_obj   = datetime.strptime(end_str, '%H-%M-%S.%f')
    td = end_obj - start_obj
    return td >= timedelta(seconds=minLen) and td <= timedelta(seconds=maxLen)

# Actual Functions
def create_signatures(clip):
    saving_frames_per_second = min(clip.fps, frames)
...
def find_similarity(signatures, fd_to_idx, frame_list, minLen, maxLen, threshold, eval):
            ...
            if len(hash_bucket) > 1:
                ...
                for i in range(len(hash_bucket)):
                    for j in range(i+1, len(hash_bucket)):
                        ...
                        tup = tuple([cpa,cpb])
                        if not tup in found:
                            if self.is_valid(cpa,cpb,minLen,maxLen):
...
def process_clip(clip, minLen, maxLen, threshold, eval):
    signatures, fd_to_idx, frame_list = create_signatures(clip)
